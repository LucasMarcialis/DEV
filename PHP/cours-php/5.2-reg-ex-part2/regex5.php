...
if (isset($_POST['texte']))
{
    ...
    $texte = preg_replace('#\[b\](.+)\[/b\]#isU', '<strong>$1</strong>', $texte); // bb code bold
    $texte = preg_replace('#\[i\](.+)\[/i\]#isU', '<em>$1</em>', $texte); // bb code italic
    $texte = preg_replace('#\[color=(red|green|blue|yellow|purple|olive)\](.+)\[/color\]#isU', '<span style="color:$1">$2</span>', $texte); // bb code couleur
    $texte = preg_replace('#\[u\](.+)\[/u\]#', '<span style=text-decoration: underline>$0</span>', $texte); // bb code underline
    $texte = preg_replace('#[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}#', '<a href="mailto:$0">$0</a>', $texte); // email
    $texte = preg_replace('#[a-z.]+\.[a-z]{2,4}\??[a-z0-9._/-=&]*#i', 'https://$0', $texte); // url en https
    $texte = preg_replace('#(https://|http://)[a-z0-9._/-]+\??[a-z0-9._/-=&]*#i', '<a href="$0">$0</a>', $texte); // https en lien
